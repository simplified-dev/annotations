 ...
 *
 * <p>It is silent wherever the processor prints no note: on an interface, whose
 * entry points call its sibling builder; on an abstract type target, which has
    ...
    public @NotNull PsiElementVisitor buildVisitor(@NotNull ProblemsHolder holder, boolean isOnTheFly) {
        return new JavaElementVisitor() {
            ...
            public void visitAnnotation(@NotNull PsiAnnotation annotation) {
                ...
                if (!ClassBuilderConstants.withholdsEntryPointsOnly(target, names.type(), executable,
                    seeds.size())) {
                    return;
                }
