 ...
 *
 * <p>The reader is written once per side because the two source sets cannot share
 * one - the IntelliJ test framework's module layer hides {@code jdk.compiler},
 * which the apt suite requires - so the resource is the shared artifact and this
 * class is the editor half's way in. The apt half's twin lives at
 * {@code library/src/aptTest/java/dev/simplified/classbuilder/BuilderParityFixture.java}
 * and reads the same files by the same rules.
 */
